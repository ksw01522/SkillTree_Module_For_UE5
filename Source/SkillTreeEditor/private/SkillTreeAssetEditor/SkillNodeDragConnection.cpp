...
TSharedRef<FSkillNodeDragConnection> FSkillNodeDragConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FSkillNodeDragConnection> Operation = MakeShareable(new FSkillNodeDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FSkillNodeDragConnection::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
...
void FSkillNodeDragConnection::HoverTargetChanged()
{
	...
	else if (UEdGraphNode_SkillNode* TargetNodeObj = Cast <UEdGraphNode_SkillNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValideSourcePins;
		ValidateGraphPinList(/*out*/ ValideSourcePins);
...
void FSkillNodeDragConnection::OnDragged(const FDragDropEvent& DragDropEvent)
{
...
void FSkillNodeDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
...
FSkillNodeDragConnection::FSkillNodeDragConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
...
FReply FSkillNodeDragConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FSkillNodeDragConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FSkillNodeDragConnection::DroppedOnPanel(const TSharedRef<SWidget>& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
